...
--Task 2
female_over_60_percentage = FOREACH (GROUP extra_data BY cond) {
    type = (cond > 0 ? 'Less then 65kg' : 'More then 65kg');
    GENERATE type AS type, COUNT(extra_data)/(double)female_over_60_count.count * 100 as count, cond as cond;
};
